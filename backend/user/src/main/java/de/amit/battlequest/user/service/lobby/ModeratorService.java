...
import de.amit.battlequest.user.exception.ModeratorNotFoundException;
...
public class ModeratorService {
	...
	public ResponseEntity<String> create(Lobby lobby, User moderator) {
		...
		if (responseEntity.getStatusCode() == HttpStatus.OK) responseEntity = new ResponseEntity<>("Moderator wurde gesetzt", HttpStatus.CREATED);
		return responseEntity;
	}

	public ResponseEntity<String> delete(Lobby lobby) {
		if (lobby.getModerator() == null) return new ResponseEntity<>("Diese Lobby hat keinen Moderator", HttpStatus.BAD_REQUEST);
	...
	public User read(Lobby lobby) {
		...
		if (lobby.getModerator() == null) throw new ModeratorNotFoundException();
	...
	public ResponseEntity<String> update(Lobby lobby, User moderator) {
		try {
			if (lobby == null) throw new LobbyNotFoundException();
			if (moderator == null) throw new ModeratorNotFoundException();
			if (!lobby.getCode().equals(moderator.getLobby().getCode())) throw new HttpException("Moderator gehört nicht der Lobby an", HttpStatus.BAD_REQUEST);
			if (lobby.getModerator() != null && lobby.getModerator().getUuid().equals(moderator.getUuid())) throw new HttpException("Ist bereits Moderator der Lobby", HttpStatus.BAD_REQUEST);
