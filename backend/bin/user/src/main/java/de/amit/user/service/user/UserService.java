...
public class UserService {
	...
	public ResponseEntity<String> create(String username, String password) {
		...
		if (username.length() > 32)
			return new ResponseEntity<>("Username darf maximal 32 Zeichen haben", HttpStatus.BAD_REQUEST);
		if (password == null || password.equals(""))
			return new ResponseEntity<>("Passwort darf nicht leer sein", HttpStatus.BAD_REQUEST);
		if (password.length() > 32)
			return new ResponseEntity<>("Passwort darf maximal 32 Zeichen haben", HttpStatus.BAD_REQUEST);
		if (password.length() < 8)
			return new ResponseEntity<>("Passwort muss mindestens als 8 Zeichen haben", HttpStatus.BAD_REQUEST);
